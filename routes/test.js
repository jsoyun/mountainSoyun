...
queryParams += '&' + encodeURIComponent('searchWrd') + '=' + encodeURIComponent(mountainarry[0]);; /* */
queryParams += '&' + encodeURIComponent('numOfRows') + '=' + encodeURIComponent('200'); /* */
queryParams += '&' + encodeURIComponent('pageNo') + '=' + encodeURIComponent('1'); /* */
let fullurl = url + queryParams;
const result = request(fullurl, (error, response, body) => {
  if (!error && response.statusCode == 200) {
    const newmountaidata = [];
    data = JSON.parse(convert.xml2json(body, { compact: true, spaces: 4 }));
    let mountaindata = data.response.body.items.item;
    if (mountaindata.length == undefined) {
      const name = mountaindata['mntiname']['_text'];
      console.log('위조건' + name);
      newmountaidata.push(name);
      console.log('위조건' + newmountaidata);
    } else {
      let j = 0;
      while (j < mountaindata.length) {
        if (JSON.stringify(mountaindata[j]['mntitop']) != '{}') {
          const name = mountaindata[j]['mntiname']['_text'];
          console.log('아래조건' + name);
          newmountaidata.push(name);
          console.log('아래조건' + newmountaidata);
          // Mountain.create({
          //   mountainlistno,
          //   name,
          //   height,
    ...
    return console.log(newmountaidata);
  }
});
console.log(result)
